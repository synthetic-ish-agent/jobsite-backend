...
# =================================================================
# CONFIGURATION - Load sensitive data securely from environment
# =================================================================
...
if not app.secret_key:
    raise ValueError("No APP_SECRET_KEY environment variable set! A secure key is required for production.")
    ...
    if resume and allowed_file(resume.filename):
        ...
        try:
            ...
            if mail_username and mail_password:
                ...
                print("WARNING: Mail credentials not fully configured. Email skipped.")

        except Exception as e:
            print(f"Error sending application email: {e}")
            
        return jsonify({'message': 'Application submitted successfully!', 'filename': filename}), 200
    else:
        return jsonify({'error': 'Invalid file type. Only PDF, DOC, DOCX are allowed'}), 400

@app.route('/api/post-job', methods=['POST'])
def post_job():
    if 'user_id' not in session:
        return jsonify({'error': 'Unauthorized', 'message': 'Please log in to post a job.'}), 401
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if not user_record or not user_record.get('is_admin'):
                    return jsonify({
                        'error': 'Forbidden', 
                        'message': 'Only designated administrators are allowed to post jobs.'
                    }), 403
        ...
        if not all([title, company, location, job_type, description, requirements]):
            return jsonify({'error': 'Missing required fields (Title, Company, Location, Type, Description, Requirements)'}), 400

        if not application_email and not phone_number:
            return jsonify({'error': 'You must provide either an application email or a phone number for job seekers to apply.'}), 400
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    (title, company, location, job_type, salary, description, 
                     requirements, application_email, phone_number, current_user_id)
                )
...
def get_jobs():
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute("SELECT id, title, company, location, type, salary, description, requirements, application_email, phone_number, poster_id, posted_date FROM jobs ORDER BY posted_date DESC")
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                # Automatically grant admin status if this is the very first registered user in the database
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if poster_id_from_db is not None and poster_id_from_db != current_user_id:
                    return jsonify({'error': 'Forbidden. You are not authorized to delete this job.'}), 403
...
def edit_job(job_id):
    ...
    if not application_email and not phone_number:
        return jsonify({'error': 'Either application email or phone number is required'}), 400
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if poster_id_from_db is not None and poster_id_from_db != current_user_id:
                    return jsonify({'error': 'Forbidden. You are not authorized to edit this job.'}), 403
                ...
                sql = """
                    ...
                    WHERE id = %s
                """
                cursor.execute(sql, (title, company, location, job_type, salary, description,
                                   requirements, application_email, phone_number, job_id))
                conn.commit()
...
def edit_job_page(job_id):
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute("SELECT id, title, company, location, type, salary, description, requirements, application_email, phone_number, poster_id FROM jobs WHERE id = %s", (job_id,))
                job = cursor.fetchone()
            ...
            with get_db_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if recovery_type == 'password':
                        cur.execute("SELECT id, email, full_name FROM users WHERE email = %s", (contact_value,))
                        user = cur.fetchone()
                        ...
                        if user:
                            ...
                            cur.execute("UPDATE users SET reset_token = %s, reset_token_expiration = %s WHERE id = %s",
                        ...
                        cur.execute("SELECT full_name FROM users WHERE email = %s", (contact_value,))
                        user_name = cur.fetchone()
                        
                        if user_name:
                            flash(f'The full name (username) associated with that email is: {user_name["full_name"]}', 'success')
                        else:
                            flash('Email address not found.', 'danger')
...
def reset_password(token):
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT id FROM users WHERE reset_token = %s AND reset_token_expiration > %s",
                            (token, datetime.now()))
                user = cur.fetchone()
            ...
            if request.method == 'POST':
                ...
                with conn.cursor() as cur:
                    cur.execute("UPDATE users SET password = %s, reset_token = NULL, reset_token_expiration = NULL WHERE id = %s",
...
def handle_disconnect():
    ...
    emit('message', {'sender': 'System', 'text': f'{username} has left the chat.'}, broadcast=True)
...
def on_join(data):
    ...
    emit('message', {'sender': 'System', 'text': f'{username} has joined the chat.'}, broadcast=True)
